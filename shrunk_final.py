...
print("shrunk_final started.") 
starter_csv = sys.argv[1]
ender_csv = sys.argv[2]
the_directory = sys.argv[3]
that_param = sys.argv[4]
if(that_param != "overall"):
    that_param = that_param.split(",")
else:
    that_param = []
...
for i in the_names:
    if(len(that_param) != 0):
        if(i not in that_param):
            continue
    lb_reduce = []
    ub_reduce = []
    with open(the_directory+"/"+i+".txt") as file:
...
for i_name in range(len(the_names)):
    if(the_names[i_name] not in that_param):
        f.write(str(the_names[i_name]) + ","+str(the_dictionary_lb[the_names[i_name]])+","+str(the_dictionary_ub[the_names[i_name]]))
...
print("Shrunk_final completed successfully.")
