...
while(FINISHED == False and super_counter < 3):
    ...
    the_result1 = ""
    the_result1 = subprocess.run('./M1_all_submit_testing.sh', stdout=subprocess.PIPE, universal_newlines=True, shell=True)
    ...
    print("yeh")
    print(str(os.getcwd()))
    the_result1 = subprocess.run('sbatch batchShifter_testing.slr '+ THE_PATH+job_id, stdout=subprocess.PIPE, universal_newlines=True, shell=True)
    ...
    subprocess.run("cp *.txt "+ str(super_counter)+"_trail_testing_"+str(trail_num)+"/", shell=True)
    subprocess.run("cp *.png "+ str(super_counter)+"_trail_testing_"+str(trail_num)+"/", shell=True)
    subprocess.run("cp *.npy "+ str(super_counter)+"_trail_testing_"+str(trail_num)+"/", shell=True)
