...
FINISHED = False
super_counter = 0
while(FINISHED == False or super_counter != 2):
    FINISHED = True
    the_vals = []
    ...
    for iq in range(len(the_names)):
        ...
        the_new_lb.append(str(the_dictionary_lb[the_names[i]]))
        the_new_ub.append(str(the_dictionary_ub[the_names[i]]))
    ...
    os.chdir(currentDir)
    subprocess.run("cp /pscratch/sd/s/sdough/tmp_neuInv/bbp3/L5_TTPC1cADpyr0/"+job_id+"/the_data.npz /pscratch/sd/s/sdough/NEW_DATA_OCT_25_2023/the_data.npz",shell=True)
